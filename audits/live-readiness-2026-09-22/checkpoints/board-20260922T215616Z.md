...
- **AVANCE 21:45-22:00Z (orquestador)**:
  ...
  - `unit_tests`+`dep_tree` — run GH 35788443570 SUCCESS (43 tests passed); registry ya
    fresco 09-13 (<30d) ✓.
  - `variance_benchmark` 🔴 FALLÓ re-run VPS (pid 2237284): JSONL export emitía
    `amount_in_wei` como entero JSON; harness exige String (u256-as-string,
    variance_benchmark.rs:88) → panic primera línea. **FIX RAÍZ PR #630** (export
    `::text`, 1 línea) — merge en espera de CI; tras merge: pull VPS + re-run script.
