...
- **ACTIVO en runtime (20:15Z)**: deploy del searcher recreó el proceso con el pool de 3
  (log `rpc_pool.ready ... count=3`). Verificación de efecto en curso (métricas
  arbx_v3_quote_total + histograma archiver post-restart). Si batch_call_error persiste
  alto → knob ARBX_V3_QUOTE_BATCH_SIZE (100→25, gas multicall 15M→3.75M bajo caps).
