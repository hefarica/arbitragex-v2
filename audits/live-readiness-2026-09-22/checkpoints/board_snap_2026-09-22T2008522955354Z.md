...
## WO-LR22.11 — ENRICHER-SILENT-01: token-enricher broken-pipe zombie desde 10:19Z — 🟡 REVIVED, DURABLE PENDIENTE
...
- **Revive operacional (20:06Z)**: `docker compose ... restart token-enricher` → grupo
  `enricher` drenado (pending 0, last-delivered al presente, batch_read fluyendo).
- **Durable PENDIENTE**: (a) reconexión real del consumer Redis (recrear cliente en error
  fatal, no retry sobre socket muerto); (b) healthcheck que exponga frescura de consumo
  (p.ej. timestamp del último batch_read). PR propio (no mezclado con #629).
...
## WO-LR22.12 — FUNNEL-V3-RPC: 80% v3_quote_unavailable = saturación RPC único chain-1 — 🟡 CONFIG LANDED (verificando)
...
- **Verificación pendiente post-deploy**: delta rpc_ok/rpc_error, caída del share
  v3_quote_unavailable en el histograma del archiver, y (objetivo) primeras filas
  viables → paper_trade_runs. Si batch_call_error persiste alto → knob
  ARBX_V3_QUOTE_BATCH_SIZE (100→25, gas multicall 15M→3.75M bajo caps de proveedor).
