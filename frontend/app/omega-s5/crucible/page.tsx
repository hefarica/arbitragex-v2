...
import { CrucibleStatusResponseSchema, type CrucibleStatusRow } from "@/lib/schemas";
import { FrontierStateView, type FrontierStateMessage } from "@/components/FrontierStateView";

// OMEGA-8 / M5 Fase 6: Zod-parsed crucible status with fail-honest unions.
export default function OmegaS5CruciblePage() {
  const [rows, setRows] = useState<CrucibleStatusRow[]>([]);
  const [msg, setMsg] = useState<FrontierStateMessage>({ kind: "loading", resource: "crucible status" });

  useEffect(() => {
    let cancelled = false;
    (async () => {
      const r = await getValidated("/api/crucible/status", CrucibleStatusResponseSchema, { withCredentials: true });
      if (cancelled) return;
      if (r.kind === "ok") {
        const networks = r.data.networks ?? [];
        setRows(networks);
        if (networks.length === 0) setMsg({ kind: "empty", resource: "crucible status" });
        else setMsg({ kind: "loading", resource: "" });
      } else {
        setMsg({ kind: r.kind, detail: "detail" in r ? r.detail : String(r.kind), resource: "crucible status" });
      }
    })();
    return () => {
      cancelled = true;
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.chain_id} className="border-b border-border/50">
                  ...
                  <td>
                    {r.success_rate_pct.toFixed(2)}% ({r.resolutions_success}/{r.resolutions_total})
                  </td>
