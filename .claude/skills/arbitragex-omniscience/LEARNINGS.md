...
### Misión: Live-Engineering Overlay 2026-09-12 (instalación skill + integración omniscience)
- [ARBX] Directiva operador: objetivo **testnet-live + mainnet-live AUTORIZADO**. Instalada `.claude/skills/arbx-live-engineering/` (14 archivos, SHA256SUMS 19/19 OK, SKILL.md idéntico al .md de referencia) y añadido **§10 de supersession** en omniscience (proyecto + espejo global) + §9.1.7 superpuesto. La activación financiera (firma/broadcast de valor) sigue operador-only — parte del mandato, no restricción.
- [GEN] Paquetes de otro agente: verificar el SHA256SUMS.json del paquete ANTES de instalar. Gotcha PowerShell: el JSON es un MAPA path→hash — iterar `PSObject.Properties` (Name=path, Value=hash), no como array de objetos; y comparar el SKILL.md del ZIP contra el .md entregado aparte (hash match = cadena de custodia intacta).
- [GEN] Aportes de agentes externos (Codex): sus aserciones de "publicado/verificado" se contrastan contra la API ANTES de actuar — en esta misión el HEAD 13479bcd tenía 4 checks pendientes cuando el reporte decía "seguían pendientes al consultar" (correcto), pero el estado cambia en minutos: siempre re-chequear.
