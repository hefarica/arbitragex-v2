...
### Misión: mapa de canales 24 contenedores + alineación doctrinal mainnet-live 2026-09-25
- [ARBX] **Gate de CI fallido sobre un SHA desplegado → verificar ancestría del fix antes de diagnosticar flake**: Pipeline Integrity Gate falló sobre 882bac0c porque su fix (6b354f2a, PR #670 abierto: DB name arbitragex_v2→arbitragex) NO era ancestro del SHA desplegado (`git merge-base --is-ancestor` = no).
- [ARBX] **Los contadores acumulados de quote V3 no discriminan clase de error**: tras #668 (Fase 3d), unary 71% error / batch 97% error pero neg-cache 196k hits y canary previo 93% cache-served. Descomponer por outcome (`arbx_v3_quote_total{outcome}`: timeout/429 vs fee-tier/pool no cotizable vs liquidity=0) ANTES de elegir fix; el backfill sobre pools vacías violaría RULE 00.
- [GEN] **Selección vacía en un ask de opciones ≠ aprobación**: el estado permanece sin cambios y se re-ofrece la decisión. Sesión sin herramientas Hermes montadas = gap documentado en el entregable, jamás simulado.
- [GEN] **Primacía epistémica (orden del operador 2026-09-25)**: el conocimiento de las skills/doctrina PREVALECE como base; la observación en tiempo real lo AJUSTA con evidencia registrada. La doctrina no se borra sin documentar la orden que la reemplaza (no borrar historia, §10.2).
