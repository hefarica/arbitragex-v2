...
impl WsChainClient {
    ...
    pub async fn subscribe_pending(&self) -> anyhow::Result<impl Stream<Item = H256> + Send + '_> {
        // B-02 fix: PublicNode and other non-Alchemy providers send
        ...
        //
        // Fix: subscribe via the raw RPC method. The subscription stream
        // yields serde_json::Value items directly (R = Value, which impls
        // Deserialize). We extract the hash from either:
        //   (a) bare hex string:  "0xabcd..."  (Alchemy/Infura format)
        //   (b) JSON object:      {"hash": "0xabcd..."} (PublicNode/drpc format)
        let sub: ethers::providers::SubscriptionStream<
            '_,
            ethers::providers::Ws,
            serde_json::Value,
        > = self
        ...
        Ok(sub
            .map(move |item: serde_json::Value| {
                match item {
                    serde_json::Value::String(s) => s.parse::<H256>().ok(),
                    serde_json::Value::Object(map) => {
                        map.get("hash").and_then(|v| v.as_str()).and_then(|s| s.parse::<H256>().ok())
                    }
                    other => {
                        tracing::trace!(
                            event = "ws.pending_unexpected_format",
                            chain_id,
                            value_type = %other,
                            "unexpected pending-tx format from WS provider"
                        );
                        None
                    }
                }
