...
impl Usd {
    pub fn parse(s: &str) -> Result<Self, String> {
        if s.len() > 512 {
            return Err("usd_input_too_long".into());
        }
        ...
        BigDecimal::from_str(s)
            .map(Self)
            .map_err(|_| "invalid_usd_decimal".into())
    }
    pub fn zero() -> Self {
...
pub struct QuotedPlan {
    ...
    pub incoming: Vec<MonetaryFlow>,
    pub outgoing: Vec<MonetaryFlow>,
...
pub struct PolicyView {
    pub enabled: bool,
    pub capital_cap_usd: String,
    pub min_profit_usd: Option<String>,
    pub max_gas_usd: Option<String>,
    pub snapshot_id: String,
    pub price_revision: String,
    pub policy_revision: String,
...
fn failure(code: &str) -> Value {
    json!({"status":"DATA_GAP","candidate_eligible":false,"net_profit_usd":null,"reason":code,"repairs":[{"field":"native_binding","reason":code}]})
}
...
fn amount_is_canonical(s: &str) -> bool {
    !s.is_empty() && s.bytes().all(|b| b.is_ascii_digit()) && (s == "0" || !s.starts_with('0'))
}
fn repairs_push(repairs: &mut Vec<Value>, field: &str, reason: &str) {
    repairs.push(json!({"field":field,"reason":reason}));
}
fn sum_flows(flows: &[MonetaryFlow]) -> Result<Usd, String> {
    flows.iter().try_fold(Usd::zero(), |sum, f| {
        if f.evidence_id.is_empty() {
            return Err(format!("missing_flow_evidence:{}", f.name));
        }
        let value = Usd::parse(&f.usd)?;
        if value.is_negative() {
            return Err("flow_amount_must_be_nonnegative".into());
        }
        sum.checked_add(&value)
    })
}
fn numeric_output(v: &Value) -> bool {
    match v {
        Value::Number(n) => n.as_f64().is_some_and(f64::is_finite),
        Value::Array(a) => !a.is_empty() && a.iter().all(numeric_output),
        _ => false,
    }
}
fn operator_gate(spec: &Value, evidence: &Value, repairs: &mut Vec<Value>) {
    let Some(requirements) = spec.get("operator_requirements").and_then(Value::as_array) else {
        repairs_push(repairs, "operators", "missing_manifest_roles");
        return;
    };
    for requirement in requirements {
        if requirement["role"] == "N/A" {
            continue;
        }
        let Some(id) = requirement["id"].as_u64() else {
            repairs_push(repairs, "operators", "invalid_operator_id");
            continue;
        };
        let key = id.to_string();
        let row = &evidence["operators"][&key];
        let status = row["status"].as_str().unwrap_or("MISSING");
        let valid_computed = status == "COMPUTED"
            && row.get("value").is_some_and(numeric_output)
            && row["evidence_id"].as_str().is_some_and(|s| !s.is_empty());
        // Native exact/equivalent algorithm receipts must name the role/phase.
        let equivalent = status == "EQUIVALENT"
            && row["equivalent_for"].as_u64() == Some(id)
            && row["evidence_id"].as_str().is_some_and(|s| !s.is_empty())
            && row["method"].as_str().is_some_and(|s| !s.is_empty());
        let optional_disabled = status == "DISABLED"
            && requirement["requirement"] == "OPTIONAL_SUPPORT"
            && row["reason"] == "operator_switch_off";
        if !(valid_computed || equivalent || optional_disabled) {
            repairs_push(repairs, &format!("operators.{id}"), status);
        }
    }
}
/// Economic evaluation after quote. Does not run a broadcast or set Sim PASS.
pub fn economic_check(
    svc: &dyn AgentServices,
    ctx: &Value,
    spec: &Value,
    candidate: &Value,
    quote_value: &Value,
    ...
    let quote: QuotedPlan = match serde_json::from_value(quote_value.clone()) {
        Ok(q) => q,
        Err(_) => {
            return json!({"status":"DATA_GAP","candidate_eligible":false,"net_profit_usd":null,"reason":"invalid_quote_contract","context_id":ctx["context_id"],"plan_id":candidate["plan_id"],"plan_hash":candidate["plan_hash"],"candidate":candidate,"quote":quote_value,"operators":evidence})
        }
    };
    let policy = match svc.policy(ctx, spec) {
        Ok(p) => p,
        Err(e) => {
            let mut f = failure(&e);
            f["quote"] = quote_value.clone();
            f["legs"] = json!(quote.legs);
            f["operators"] = evidence.clone();
            f["candidate"] = candidate.clone();
            return f;
        }
    };
    for (field, actual, expected) in [
        (
            "context_id",
            quote.context_id.as_str(),
            ctx["context_id"].as_str().unwrap_or(""),
        ),
        (
            "snapshot_id",
            quote.snapshot_id.as_str(),
            policy.snapshot_id.as_str(),
        ),
        (
            "price_revision",
            quote.price_revision.as_str(),
            policy.price_revision.as_str(),
        ),
        (
            "policy_revision",
            quote.policy_revision.as_str(),
            policy.policy_revision.as_str(),
        ),
        (
            "plan_id",
            quote.plan_id.as_str(),
            candidate["plan_id"].as_str().unwrap_or(""),
        ),
        (
            "plan_hash",
            quote.plan_hash.as_str(),
            candidate["plan_hash"].as_str().unwrap_or(""),
        ),
    ] {
        if actual.is_empty() || actual != expected {
            repairs_push(&mut repairs, field, "context_revision_mismatch");
        }
    }
    if !["LIVE_MAINNET", "TESTNET", "PAPER_SHADOW"].contains(&policy.execution_mode.as_str()) {
        repairs_push(&mut repairs, "execution_mode", "unknown_mode");
    }
    if !amount_is_canonical(&quote.amount_in_raw) {
        repairs_push(&mut repairs, "amount_in_raw", "noncanonical_integer");
    }
    if quote.status != "COMPUTED" {
        repairs_push(&mut repairs, "quote", "incomplete_quote");
    }
    if !["before_financing", "retained_after_repayment"].contains(&quote.profit_basis.as_str()) {
        repairs_push(&mut repairs, "profit_basis", "unknown_profit_basis");
    }
    if ![
        "atomic_quote",
    ...
    .contains(&quote.economic_kind.as_str())
    {
        repairs_push(&mut repairs, "economic_kind", "unknown_economic_kind");
    }
    if spec["logic"] == "carry_projection" && quote.economic_kind != "expected_carry" {
        repairs_push(
            &mut repairs,
            "economic_kind",
            "carry_must_remain_projection",
        );
    }
    if spec["logic"] == "cross_domain" && quote.economic_kind != "non_atomic_expected" {
        repairs_push(&mut repairs, "economic_kind", "cross_domain_not_atomic");
    }
    if spec["logic"] == "closed_route" && quote.economic_kind != "atomic_quote" {
        repairs_push(&mut repairs, "economic_kind", "closed_route_kind_mismatch");
    }
    for m in &quote.missing {
        repairs_push(&mut repairs, m, "applicable_input_missing");
    }
    ...
    if ["closed_route", "post_state_route"].contains(&spec["logic"].as_str().unwrap_or("")) {
        if quote.legs.is_empty() {
            repairs_push(&mut repairs, "legs", "missing_leg_ledger");
        }
        for (i, leg) in quote.legs.iter().enumerate() {
            for k in [
                ...
                if leg[k].as_str().is_none_or(str::is_empty) {
                    repairs_push(&mut repairs, &format!("legs.{i}.{k}"), "missing_leg_field");
                }
            }
            if leg["snapshot_id"] != quote.snapshot_id {
                repairs_push(&mut repairs, &format!("legs.{i}"), "mixed_snapshot");
            }
            if i == 0 && leg["amount_in_raw"] != quote.amount_in_raw {
                repairs_push(&mut repairs, "legs.0.amount_in_raw", "amount_mismatch");
            }
            if i > 0
                && (quote.legs[i - 1]["token_out"] != leg["token_in"]
                    || quote.legs[i - 1]["amount_out_raw"] != leg["amount_in_raw"])
            {
                repairs_push(
                    &mut repairs,
                    &format!("legs.{i}"),
                    "broken_token_or_amount_continuity",
                );
            }
        }
        if let (Some(first), Some(last)) = (quote.legs.first(), quote.legs.last()) {
            if spec["logic"] == "closed_route" && first["token_in"] != last["token_out"] {
                repairs_push(&mut repairs, "legs", "not_closed");
            }
        }
    }
    // A positive USD cashflow cannot contradict the actual raw leg ledger.
    if ["closed_route", "post_state_route"].contains(&spec["logic"].as_str().unwrap_or("")) {
        if quote.incoming.len() != 1 || quote.outgoing.len() != 1 {
            repairs_push(
                &mut repairs,
                "cashflows",
                "sequential_route_requires_bound_principal_and_output",
            );
        } else if let (Some(first), Some(last)) = (quote.legs.first(), quote.legs.last()) {
            for (flow, leg, side) in [
                (&quote.outgoing[0], first, "in"),
                (&quote.incoming[0], last, "out"),
            ] {
                if let Some(v) = &flow.valuation {
                    let token_key = format!("token_{side}");
                    let amount_key = format!("amount_{side}_raw");
                    let dec_key = format!("token_{side}_decimals");
                    let expected = crate::agent_graph::token_value_usd(
                        &v.amount_raw,
                        v.token_decimals,
                        &v.price_usd,
                    );
                    let valid = leg[&token_key] == v.token_address
                        && leg[&amount_key] == v.amount_raw
                        && leg[&dec_key].as_u64() == Some(v.token_decimals as u64)
                        && leg["chain_id"].as_u64() == Some(v.chain_id)
                        && v.price_revision == quote.price_revision
                        && !v.price_evidence_id.is_empty()
                        && expected.ok().and_then(|n| Usd::parse(&n).ok())
                            == Usd::parse(&flow.usd).ok();
                    if !valid {
                        repairs_push(
                            &mut repairs,
                            &format!("cashflows.{}", flow.name),
                            "valuation_not_bound_to_ledger",
                        );
                    }
                } else {
                    repairs_push(
                        &mut repairs,
                        &format!("cashflows.{}", flow.name),
                        "missing_token_valuation_provenance",
                    );
                }
            }
            if let (Some(a), Some(b)) = (&quote.outgoing[0].valuation, &quote.incoming[0].valuation)
            {
                if a.chain_id == b.chain_id
                    ...
                    && (a.token_decimals != b.token_decimals
                        || Usd::parse(&a.price_usd).ok() != Usd::parse(&b.price_usd).ok())
                {
                    repairs_push(
                        &mut repairs,
                        "cashflows",
                        "same_asset_has_conflicting_price_or_decimals",
                    );
                }
            }
            if quote
                .capital_usd
                .as_deref()
                .and_then(|n| Usd::parse(n).ok())
                != Usd::parse(&quote.outgoing[0].usd).ok()
            {
                repairs_push(
                    &mut repairs,
                    "capital_usd",
                    "capital_differs_from_ledger_input",
                );
            }
        }
    }
    let gross = if quote.incoming.is_empty() || quote.outgoing.is_empty() {
        repairs_push(
            &mut repairs,
            "cashflows",
            "missing_incoming_or_outgoing_flow",
        );
        None
    } else {
        match (sum_flows(&quote.incoming), sum_flows(&quote.outgoing)) {
            (Ok(i), Ok(o)) => match i.checked_sub(&o) {
                Ok(g) => Some(g),
                Err(e) => {
                    repairs_push(&mut repairs, "gross_profit_usd", &e);
                    None
                }
            },
            _ => {
                repairs_push(&mut repairs, "cashflows", "invalid_or_unproven_flow");
                None
            }
        }
    };
    let mut seen = BTreeSet::new();
    let mut external = Usd::zero();
    let mut gas = Usd::zero();
    let mut costs_complete = true;
    if quote.required_cost_kinds.is_empty() {
        costs_complete = false;
        repairs_push(&mut repairs, "required_cost_kinds", "empty_cost_contract");
    }
    for line in &quote.costs {
        if quote.profit_basis == "retained_after_repayment"
            && line.kind == "financing"
            && line.treatment == "external"
        {
            costs_complete = false;
            repairs_push(
                &mut repairs,
                "costs.financing",
                "already_in_retained_spread",
            );
        }
        if quote.economic_kind == "atomic_quote"
            && line.kind == "execution_fees"
            && line.treatment == "external"
        {
            costs_complete = false;
            repairs_push(
                &mut repairs,
                "costs.execution_fees",
                "quote_already_includes_swap_fees",
            );
        }
        if !seen.insert(line.kind.clone()) {
            repairs_push(&mut repairs, "costs", "duplicate_cost_kind");
            costs_complete = false;
        }
        if line.evidence_id.is_empty() {
            repairs_push(
                &mut repairs,
                &format!("costs.{}", line.kind),
        ...
        match line.treatment.as_str() {
            "external" | "embedded" => match line.usd.as_deref().and_then(|x| Usd::parse(x).ok()) {
                Some(v) if !v.is_negative() => {
                    if line.treatment == "external" {
                        match external.checked_add(&v) {
                            Ok(x) => external = x,
                            Err(e) => {
                                costs_complete = false;
                                repairs_push(&mut repairs, "costs", &e);
                            }
                        }
                        if line.kind == "gas" {
                            gas = v;
            ...
            "not_applicable" => {
                if line.usd.is_some() || line.reason.as_deref().is_none_or(str::is_empty) {
                    costs_complete = false;
                    repairs_push(&mut repairs, "costs", "invalid_not_applicable");
                }
            }
            _ => {
                costs_complete = false;
                repairs_push(&mut repairs, "costs", "unknown_cost_treatment");
            }
        }
    }
    for name in &quote.required_cost_kinds {
        if !seen.contains(name) {
            costs_complete = false;
            repairs_push(
                &mut repairs,
                &format!("costs.{name}"),
                "required_cost_missing",
            );
        }
    }
    if quote.economic_kind == "atomic_quote" {
        for name in ["gas", "financing", "execution_fees"] {
            if !seen.contains(name) {
                costs_complete = false;
                repairs_push(
                    &mut repairs,
                    &format!("costs.{name}"),
                    "mandatory_route_cost_missing",
                );
            }
        }
    }
    let net = if costs_complete {
        gross.as_ref().and_then(|g| g.checked_sub(&external).ok())
    } else {
        None
    };
    ...
    let capital = quote
        .capital_usd
        .as_deref()
        .and_then(|s| Usd::parse(s).ok());
    let cap = Usd::parse(&policy.capital_cap_usd).ok();
    if !matches!((capital,cap),(Some(a),Some(b)) if a.is_positive() && b.is_positive() && a<=b) {
        policy_ok = false;
        repairs_push(
            &mut repairs,
            "capital_usd",
    ...
    if let Some(t) = policy.min_profit_usd.as_deref() {
        match Usd::parse(t) {
            Ok(t) => {
                if net.as_ref().is_none_or(|n| n <= &t) {
                    policy_ok = false;
                }
            }
            Err(e) => repairs_push(&mut repairs, "min_profit_usd", &e),
        }
    }
    if let Some(t) = policy.max_gas_usd.as_deref() {
        match Usd::parse(t) {
            Ok(t) => {
                if gas > t {
                    policy_ok = false;
            ...
            Err(e) => repairs_push(&mut repairs, "max_gas_usd", &e),
        }
    }
            ...
            for name in required {
                ...
                if matching.len() != 1 {
                    repairs_push(
                        &mut repairs,
                        name,
                        "missing_or_duplicate_constraint_receipt",
                    );
                    continue;
                }
                let r = matching[0];
                if r.status != "PASS"
                    || r.evidence_id.is_empty()
                    || r.snapshot_id != quote.snapshot_id
                    || r.plan_hash != quote.plan_hash
                {
                    repairs_push(
                        &mut repairs,
                        name,
                        r.reason.as_deref().unwrap_or("constraint_not_verified"),
                    );
                }
            }
        }
    ...
    if evidence["snapshot_id"] != quote.snapshot_id || evidence["plan_hash"] != quote.plan_hash {
        repairs_push(&mut repairs, "operators", "operator_context_mismatch");
    }
    if spec["conflicts"].as_array().is_some_and(|a| {
        a.iter()
            .any(|v| v == "NON_CYCLIC_NAME_VS_CLOSED_CYCLE_SPECIFIC_NOTE")
    }) {
        repairs_push(
            &mut repairs,
            "strategy_identity",
    ...
    let eligible = !observe
        ...
        && repairs.is_empty()
        && policy_ok
        && net.as_ref().is_some_and(Usd::is_positive);
...
fn to_json(d: &Dynamic) -> Result<Value, String> {
    ...
    if let Ok(v) = d.as_float() {
        return if v.is_finite() {
            ...
            Err("non_finite_dynamic".into())
        };
    }
    if let Some(v) = d.clone().try_cast::<ImmutableString>() {
        return Ok(json!(v.as_str()));
    }
    if let Some(v) = d.clone().try_cast::<Array>() {
        return v
            .iter()
            .map(to_json)
    ...
    if let Some(v) = d.clone().try_cast::<Map>() {
...
fn from_json(v: Value) -> Dynamic {
    match v {
        ...
        Value::Object(m) => m
            .into_iter()
            .map(|(k, v)| (k.into(), from_json(v)))
            .collect::<Map>()
            .into(),
    }
}
fn err(e: String) -> Box<EvalAltResult> {
    e.into()
}
fn cv(d: Dynamic) -> Result<Value, Box<EvalAltResult>> {
    to_json(&d).map_err(err)
}
...
pub fn register(engine: &mut Engine, svc: Arc<dyn AgentServices>) {
    ...
    engine.register_fn(
        "agent_v4_discover",
        move |ctx: Dynamic, spec: Dynamic| -> Result<Dynamic, Box<EvalAltResult>> {
            let (c, m) = (cv(ctx)?, cv(spec)?);
            Ok(from_json(
                s.discover(&c, &m).unwrap_or_else(|e| failure(&e)),
            ))
        },
    );
    let s = svc.clone();
    engine.register_fn(
        ...
        move |ctx: Dynamic,
            ...
            let (c, m, p) = (cv(ctx)?, cv(spec)?, cv(candidate)?);
            Ok(from_json(match s.quote(&c, &m, &p) {
                Ok(q) => serde_json::to_value(q).map_err(|_| err("quote_encode_failed".into()))?,
                Err(e) => failure(&e),
            }))
        },
    );
    ...
    engine.register_fn(
        ...
        move |ctx: Dynamic,
            ...
            let (c, m, p) = (cv(ctx)?, cv(spec)?, cv(candidate)?);
            Ok(from_json(
                s.operators(&c, &m, &p).unwrap_or_else(|e| failure(&e)),
            ))
        },
    );
    let s = svc.clone();
    engine.register_fn(
        ...
        move |ctx: Dynamic,
            ...
            let names: Vec<String> =
                serde_json::from_value(r).map_err(|_| err("invalid_requirement_names".into()))?;
            Ok(from_json(economic_check(
                s.as_ref(),
                &c,
                &m,
                &p,
    ...
    engine.register_fn(
        "agent_v4_money_compare",
        |a: &str, b: &str| -> Result<i64, Box<EvalAltResult>> {
            let a = Usd::parse(a).map_err(err)?;
            let b = Usd::parse(b).map_err(err)?;
            Ok(if a < b {
                -1
            } else if a > b {
                1
...
mod tests {
    use super::*;
    #[test]
    fn exact_money_round_trip() {
        for s in ["0", "-0.5", "2.000000000000000001", "99999999999999999999"] {
            assert_eq!(Usd::parse(s).unwrap().text(), s);
        }
    }
    #[test]
    fn money_rejects_invalid() {
        for s in ["", "NaN", "1e3", " 1", "+1", "1..2"] {
            assert!(Usd::parse(s).is_err(), "{s}");
        }
    }
    #[test]
    fn loss_is_preserved() {
        assert_eq!(
            Usd::parse("99.48")
                .unwrap()
                .checked_sub(&Usd::parse("100").unwrap())
                .unwrap()
                .text(),
            "-0.52"
        );
