...
pub struct ProposalV4 {
    ...
    pub gross_profit_usd: Option<String>,
    ...
    #[serde(default)]
    pub economic_kind: Option<String>,
    #[serde(default)]
    pub legs: Vec<Value>,
    #[serde(default)]
...
impl ProposalV4 {
    pub fn parse(value: Value) -> Result<Self, String> {
        let p: Self =
            serde_json::from_value(value).map_err(|e| format!("invalid_v4_result:{e}"))?;
        if p.contract_version != "arbx.cartridge.agent/4"
            || p.mev_id.is_empty()
            || p.manifest_digest.is_empty()
            || p.context_id.is_empty()
        {
            return Err("invalid_v4_identity".into());
        }
        if p.approved_for_execution {
            return Err("cartridge_cannot_self_authorize_execution".into());
        }
        for amount in [&p.gross_profit_usd, &p.net_profit_usd]
            .into_iter()
            .flatten()
        {
            Usd::parse(amount)?;
        ...
        if p.candidate_eligible {
            if p.status != "CANDIDATE"
                ...
                || p.net_profit_usd
                    ...
                    .is_none_or(|n| !n.is_positive())
            {
                return Err("candidate_economics_inconsistent".into());
            }
            for f in [
                &p.plan_id,
                &p.plan_hash,
                ...
                if f.as_deref().is_none_or(str::is_empty) {
                    return Err("candidate_missing_plan_binding".into());
                }
            }
            if p.legs.is_empty() {
                return Err("candidate_missing_action_ledger".into());
            }
        }
        Ok(p)
    }
    ...
    pub fn plan_lookup_key(&self) -> Result<(&str, &str, &str), String> {
        if !self.candidate_eligible {
            return Err("not_an_economic_candidate".into());
        }
        Ok((
            self.plan_hash.as_deref().ok_or("missing_plan_hash")?,
            self.snapshot_id.as_deref().ok_or("missing_snapshot")?,
            self.amount_in_raw.as_deref().ok_or("missing_amount")?,
        ))
    }
}
