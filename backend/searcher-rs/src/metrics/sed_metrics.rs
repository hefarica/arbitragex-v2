...
use prometheus::{exponential_buckets, histogram_opts, opts, Counter, Gauge, Histogram, Registry};
...
impl SedMetrics {
    fn new() -> Self {
        ...
        registry
            .register(Box::new(mempool_connected.clone()))
            .unwrap();
        registry
            .register(Box::new(mempool_tx_received.clone()))
            .unwrap();
        registry
            .register(Box::new(mempool_normalization_latency_us.clone()))
            .unwrap();
        registry
            .register(Box::new(reserve_fetch_latency_ms.clone()))
            .unwrap();
        registry
            .register(Box::new(sed_pipeline_latency_ms.clone()))
            .unwrap();
        ...
        Self {
            ...
            mempool_connected,
            mempool_tx_received,
            mempool_tx_missing: Counter::with_opts(opts!(
                "sed_mempool_tx_missing_total",
                "TX hashes without full data"
            ))
            .unwrap(),
            mempool_rpc_errors: Counter::with_opts(opts!(
                "sed_mempool_rpc_errors_total",
                "RPC errors in mempool"
            ))
            .unwrap(),
            mempool_normalization_latency_us,
            reserve_fetch_success: Counter::with_opts(opts!(
                "sed_reserve_fetch_success_total",
                "Successful reserve fetches"
            ))
            .unwrap(),
            reserve_fetch_latency_ms,
            sed_opportunities_detected: Counter::with_opts(opts!(
                "sed_opportunities_detected_total",
                "Opportunities detected"
            ))
            .unwrap(),
            sed_opportunities_rejected: Counter::with_opts(opts!(
                "sed_opportunities_rejected_total",
                "Opportunities rejected"
            ))
            .unwrap(),
            sed_simulations_run: Counter::with_opts(opts!(
                "sed_simulations_run_total",
                "Simulations executed"
            ))
            .unwrap(),
            sed_simulations_success: Counter::with_opts(opts!(
                "sed_simulations_success_total",
                "Simulations successful"
            ))
            .unwrap(),
            sed_pipeline_latency_ms,
            killswitch_triggered: Counter::with_opts(opts!(
                "sed_killswitch_triggered_total",
                "Kill-switch activations"
            ))
            .unwrap(),
            killswitch_blocks_prevented: Counter::with_opts(opts!(
                "sed_killswitch_blocks_prevented_total",
                "Executions blocked by kill-switch"
            ))
            .unwrap(),
        }
    }
