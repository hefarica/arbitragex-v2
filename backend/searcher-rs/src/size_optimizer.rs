...
mod tests {
    ...
    fn br06_closed_form_matches_kernel_onchain_fixture() {
        ...
        for probe in geom_probes(x_lo, x_hi, 200) {
            let net =
                eval_2leg_profit(probe, &hop_a, &hop_b, BR06_FEE_V2_BPS, BR06_FEE_V2_BPS) as f64;
            assert!(
    ...
    fn br06_symmetric_pools_closed_form_sign_and_gamma_one_boundary() {
        ...
        assert!(
            profit <= 0,
            "symmetric pools must yield no profit, got {profit}"
        );
        ...
        let (_, profit0) =
            golden_section_search_2leg(U256::from(1u64), br06_v2_r1_weth(), &hop, &hop, 0, 0, 25);
        assert!(
            profit0 <= 0,
    ...
    fn br06_fee_enters_via_geometric_mean_threshold_not_split() {
        ...
        for (fa, fb) in [(30u32, 30u32), (5, 30), (0, 0)] {
            ...
            let (_, profit) =
                golden_section_search_2leg(U256::from(1u64), x1t, &ha_t, &hb_t, fa, fb, 25);
            assert_eq!(
                profit > 0,
                cf.numerator > 0.0,
        ...
        let cf_x =
            br06_closed_form_2leg(1e18, 2e18, 1e18, 2e18, BR06_FEE_V2_BPS, BR06_FEE_V3_5_BPS);
        let cf_y =
            br06_closed_form_2leg(1e18, 2e18, 1e18, 2e18, BR06_FEE_V3_5_BPS, BR06_FEE_V2_BPS);
    ...
    fn br06_kelly_caps_exact_amount_and_never_exceeded() {
        ...
        match SizeOptimizer::apply_kelly_constraints(
            ...
            OptimizeOutcome::Sized(s) => {
                ...
                assert!(
                    s.net_negative,
                    "scaled net ≤ 0 must be flagged net_negative"
                );
        ...
        match SizeOptimizer::apply_kelly_constraints(
            ...
            OptimizeOutcome::Sized(s) => {
                ...
                let expected = f64_to_u256_clamped(((3_000.0 * f_capped) / 3000.0 * 1e18).floor());
