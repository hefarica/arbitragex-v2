...
mod tests {
    ...
    // ═══════════════════════════════════════════════════════════════════
    // BR-06 (2026-09-07) — property tests: convex CFMM sizing vs the
    // closed-form quadratic argmax.
    //
    // Canon applied (charter: cite the principle, don't re-derive the world):
    // - Angeris-Evans-Chitra-Boyd, "Optimal Routing for CFMMs" (EC'22,
    //   arXiv:2204.05238): CFMM routing is a convex program; a 2-leg
    //   same-pair cycle is the single-cycle special case with a closed form.
    // - Diamandis et al., "Efficient Algorithm for Optimal Routing Through
    //   CFMMs" (FC'23, arXiv:2302.04938): each CFMM contributes a small
    //   closed-form single-market arb subproblem — Δ* below IS that answer.
    // - arXiv:2406.16573 (cycle sizing): optimal loop size from dNet/dΔ = 1
    //   on the concave output curve — the FOC solved in closed form here.
    // - Xi & Moallemi (arXiv:2607.20762): marginal-price equalization is the
    //   KKT condition; stale state (not algorithm) dominates realized loss —
    //   hence every on-chain fixture below is block-stamped.
    // - Bartoletti et al. Lean certification (arXiv:2510.14480): correctness
    //   as a theorem, not a backtest — these tests assert the FORMULA, never
    //   a recorded output.
    //
    // Closed form (derived independently here; matches BR-05+06-VERIFY.md
    // §2.2): cycle x→[A:(x1,y1), fee γ1]→y→[B:(y2,x2), fee γ2]→x with
    // o1 = γ1·Δ·y1/(x1+Δ) and out2 = γ2·x2·o1/(y2+o1):
    //
    //   dNet/dΔ = γ1γ2·x1·y1·x2·y2 / [(x1+Δ)²·(y2+o1)²] = 1
    //   ⇒ (x1+Δ)·(y2+o1) = √(γ1γ2·x1·y1·x2·y2) =: K
    //   ⇒ Δ* = (K − x1·y2) / (y2 + γ1·y1)     [linear in Δ after the √]
    //
    // CORRECTION to BR-05+06-VERIFY §2.2 check 3 (refuted below with an
    // explicit counterexample): Δ* is NOT invariant under γ1↔γ2 exchange —
    // γ1 appears ALONE in the denominator. What IS invariant is the
    // numerator K − x1·y2 (fees enter only via √(γ1γ2)) and therefore the
    // profitability threshold γ1γ2·(y1·x2)/(x1·y2) > 1: the fee PRODUCT,
    // not its split, gates the cycle.
    //
    // ── Fixture provenance (RULE 00: every on-chain claim is citable) ──
    // Read via ONE read-only JSON-RPC batch (eth_call @ "latest") against
    // https://ethereum-rpc.publicnode.com on 2026-09-09, eth_blockNumber in
    // the SAME batch = 25,937,800:
    //   - pair 0xB4e16d0168e52d35CaCD2c6185b44281Ec28C9Dc (Uniswap V2
    //     USDC/WETH): token0() = 0xA0b8…eB48 (USDC), token1() = 0xC02a…6Cc2
    //     (WETH) — address identity verified ON-CHAIN, not from memory.
    //     getReserves() = (10120018743285, 4046564278775630236987).
    //     Cross-venue sanity: implied ≈ $2,501/WETH vs V3 slot0 ≈ $2,495.
    //   - pool 0x88e6A0c2dDD26FEEb64F039a2c41296FcB3f5640 (Uniswap V3
    //     USDC/WETH 0.05%): fee() = 500 pips = 5 bps (ON-CHAIN fee read —
    //     ROUTES_CROWN_JEWEL; the 5-bps leg value below is this call). Also
    //     captured for the report: slot0.sqrtPriceX96 =
    //     1586264817113937121424052718914424, liquidity = 5208185248964539533,
    //     tick 198101 (the V2 kernel here consumes no V3 state).
    //   - USDC.decimals() = 6, WETH.decimals() = 18 (same batch).
    //   - 30 bps (V2 legs): UniswapV2Pair PROTOCOL constant (997/1000 in the
    //     pair bytecode — NOT a state variable; bytecode semantics, same
    //     constant as the amm_math.rs doctrine header and the motor default
    //     `legs[i].fee_bps.unwrap_or(30)` at the call site).
    //   - The "second venue" of each 2-leg cycle is a DECLARED synthetic
    //     dislocation of the same on-chain reserves (one side × m/1000,
    //     integer floor). Rule 00: the base is real and cited; the
    //     dislocation is a labeled sweep parameter — nothing here claims to
    //     be two real venues.

    /// Block height of the on-chain reads backing the BR-06 fixtures.
    const BR06_BLOCK: u64 = 25_937_800;

    /// V2 USDC/WETH getReserves r0 — USDC side (6 decimals), block 25,937,800.
    fn br06_v2_r0_usdc() -> U256 {
        U256::from_dec_str("10120018743285").unwrap()
    }

    /// V2 USDC/WETH getReserves r1 — WETH side (18 decimals), block 25,937,800.
    fn br06_v2_r1_weth() -> U256 {
        U256::from_dec_str("4046564278775630236987").unwrap()
    }

    /// 30 bps = UniswapV2Pair protocol constant (bytecode 997/1000).
    const BR06_FEE_V2_BPS: u32 = 30;

    /// 5 bps = V3 fee() = 500 pips read on-chain (pool 0x88e6…5640,
    /// block 25,937,800) — a fee the CHAIN told us, per doctrine.
    const BR06_FEE_V3_5_BPS: u32 = 5;

    /// Closed-form reference for the continuous relaxation of the kernel's
    /// 2-leg curve. `hop_a = (x1, y1)`, `hop_b = (y2, x2)` exactly as
    /// `eval_2leg_profit` orients them.
    struct Br06ClosedForm {
        /// Δ* in wei of the cycle's input token.
        delta_star: f64,
        /// Net(Δ*) = out2(Δ*) − Δ* at the continuous optimum.
        net_star: f64,
        /// K − x1·y2 — sign gates profitability; fee-swap invariant.
        numerator: f64,
    }

    fn br06_closed_form_2leg(
        x1: f64,
        y1: f64,
        y2: f64,
        x2: f64,
        ...
        let g1 = 1.0 - f64::from(fee_a) / 10_000.0;
        let g2 = 1.0 - f64::from(fee_b) / 10_000.0;
        let k = (g1 * g2 * x1 * y1 * x2 * y2).sqrt();
        let numerator = k - x1 * y2;
        let delta_star = numerator / (y2 + g1 * y1);
        let o1 = g1 * delta_star * y1 / (x1 + delta_star);
        let out2 = g2 * x2 * o1 / (y2 + o1);
        Br06ClosedForm {
            delta_star,
            net_star: out2 - delta_star,
            numerator,
        }
    }

    /// The kernel's own bracket semantics (motor call site): x_hi =
    /// min(cap_wei, reserve_in_a) with a non-binding cap ⇒ reserve_in_a.
    fn br06_hop_pair(m_mille: u64) -> (Vec<(U256, U256)>, Vec<(U256, U256)>) {
        let hop_a = vec![(br06_v2_r1_weth(), br06_v2_r0_usdc())];
        let x2 = br06_v2_r1_weth() * U256::from(m_mille) / U256::from(1000u64);
        let hop_b = vec![(br06_v2_r0_usdc(), x2)];
        (hop_a, hop_b)
    }

    /// BR-06 gap #1 (closed form): on the block-stamped on-chain fixture the
    /// golden-section argmax must sit ON the analytic Δ*, its integer net
    /// must be ≤ Net(Δ*) (concavity envelope: two `v2_amount_out` floors can
    /// only lose wei) and within rounding of it, and EVERY probe of the
    /// bucket grid must respect the same envelope.
    #[test]
    fn br06_closed_form_matches_kernel_onchain_fixture() {
        let (hop_a, hop_b) = br06_hop_pair(1050);
        let (x1, y1) = hop_a[0];
        let (y2, x2) = hop_b[0];
        let cf = br06_closed_form_2leg(
            u256_to_f64_lossy(x1),
            u256_to_f64_lossy(y1),
            u256_to_f64_lossy(y2),
            u256_to_f64_lossy(x2),
            BR06_FEE_V2_BPS,
            BR06_FEE_V2_BPS,
        );
        // Hand-derived vector (exact Python port of the formula, 2026-09-09):
        // Δ* = 4.381108229e19 wei = 43.811082 WETH, Net* = 9.472390319e17 wei.
        assert!(cf.delta_star > 0.0, "fixture must be profitable");
        assert!((cf.delta_star - 4.381_108_229e19).abs() / cf.delta_star < 1e-6);
        assert!((cf.net_star - 9.472_390_319e17).abs() / cf.net_star < 1e-6);

        let x_lo = U256::from(1u64);
        let x_hi = x1; // motor bound: min(cap, reserve_in_a), cap non-binding
        let (x_star, profit) =
            golden_section_search_2leg(x_lo, x_hi, &hop_a, &hop_b, BR06_FEE_V2_BPS, BR06_FEE_V2_BPS, 25);

        let x_star_f = u256_to_f64_lossy(x_star);
        let loc_rel = (x_star_f - cf.delta_star).abs() / cf.delta_star;
        // Golden-section 25-iter worst-case bracket: φ⁻²⁵·range/2 ≈ 1.2e16 wei
        // ⇒ 2.75e-4 relative; 2e-3 is a 7× margin over that bound.
        assert!(
            loc_rel < 2e-3,
            "kernel argmax {x_star_f:e} must sit on Δ* {:e} (rel {loc_rel:e})",
            cf.delta_star
        );

        let profit_f = profit as f64;
        assert!(
            profit_f <= cf.net_star * (1.0 + 1e-9),
            "integer net {profit_f:e} cannot exceed the continuous optimum {:e}",
            cf.net_star
        );
        assert!(
            profit_f >= cf.net_star * 0.999,
            "kernel must harvest ≥99.9% of Net*: {profit_f:e} vs {:e}",
            cf.net_star
        );

        // Envelope over a dense probe grid AND the canonical bucket grids:
        // Net(Δ) ≤ Net(Δ*) for every evaluated Δ.
        for probe in geom_probes(x_lo, x_hi, 200) {
            let net = eval_2leg_profit(
                probe,
                &hop_a,
                &hop_b,
                BR06_FEE_V2_BPS,
            ...
            assert!(
                net <= cf.net_star * (1.0 + 1e-9),
                "grid point {} violates the concavity envelope: {net:e} > {:e}",
                probe,
                cf.net_star
            );
        }
        for n in crate::amount_buckets::AMOUNT_BUCKETS_CANONICAL {
            let sweep = bucket_sweep_2leg_curve(
                n,
                x_lo,
                x_hi,
                &hop_a,
            ...
            for point in &sweep.points {
                assert!(
                    point.net_wei as f64 <= cf.net_star * (1.0 + 1e-9),
                    "bucket N={n} point {} exceeds Net*: {}",
                    point.amount_in_wei,
                    point.net_wei
                );
            }
    ...
    /// BR-06 gap #1 (sign structure, verify §2.2 checks 1-2): identical
    /// pools collapse the closed form to Δ* = U·(γ−1)/(1+γ) — negative for
    /// any fee (the honest `best: None` of the existing sweep test is the
    /// FORMULA's prediction), and exactly 0 at zero fee.
    #[test]
    fn br06_symmetric_pools_closed_form_sign_and_gamma_one_boundary() {
        let u = u256_to_f64_lossy(br06_v2_r1_weth());
        let v = u256_to_f64_lossy(br06_v2_r0_usdc());
        // hop_a = hop_b as oriented reserves of the SAME pool.
        let gamma = 1.0 - f64::from(BR06_FEE_V2_BPS) / 10_000.0;
        let exact = u * (gamma - 1.0) / (1.0 + gamma);

        let cf = br06_closed_form_2leg(u, v, v, u, BR06_FEE_V2_BPS, BR06_FEE_V2_BPS);
        assert!((cf.delta_star - exact).abs() / exact.abs() < 1e-12);
        assert!(cf.delta_star < 0.0, "identical pools: no cycle at any fee");

        // Kernel agreement: golden-section finds no positive net (R8 honest).
        let hop = vec![(br06_v2_r1_weth(), br06_v2_r0_usdc())];
        let (_, profit) = golden_section_search_2leg(
            U256::from(1u64),
            br06_v2_r1_weth(),
            &hop,
            &hop,
        ...
        assert!(profit <= 0, "symmetric pools must yield no profit, got {profit}");

        // γ=1 boundary: Δ* = 0 to f64 noise (γ=1.0 is exact in f64; the
        // residual is √-of-rounded-product ulp noise, |Δ*| ≪ 1e-15·U).
        let cf0 = br06_closed_form_2leg(u, v, v, u, 0, 0);
        assert!(
            cf0.delta_star.abs() < u * 1e-15,
            "zero-fee symmetric boundary must be Δ*=0, got {:e}",
            cf0.delta_star
        );
        let (_, profit0) = golden_section_search_2leg(
            U256::from(1u64),
            br06_v2_r1_weth(),
            &hop,
            &hop,
        ...
        assert!(
            profit0 <= 0,
            "zero-fee round trip through one pool loses to integer floors (≤0), got {profit0}"
        );
    }

    /// BR-06 gap #2 (monotonicity): Δ* = (K − x1·y2)/(y2 + γ1·y1) with
    /// K = √(γ1γ2·x1·y1·x2·y2) is STRICTLY increasing in the opposing
    /// pool's exit reserve x2 (K ∝ √x2, denominator fixed) — the argmax
    /// grows with the dislocation. The kernel's chosen size follows the
    /// closed form rung by rung, and the profitability SIGN agrees with
    /// the threshold at every rung.
    #[test]
    fn br06_sizing_monotone_in_opposing_pool_reserve() {
        let mut prev_delta: Option<f64> = None;
        let mut prev_x_star: Option<f64> = None;
        for m in [1001u64, 1007, 1010, 1020, 1030, 1050, 1080, 1120, 1200] {
            let (hop_a, hop_b) = br06_hop_pair(m);
            let (x1, y1) = hop_a[0];
            let (y2, x2) = hop_b[0];
            let cf = br06_closed_form_2leg(
                u256_to_f64_lossy(x1),
            ...
            if let Some(pd) = prev_delta {
                assert!(
                    cf.delta_star > pd,
                    "closed form must strictly increase in x2 (m={m}): {:e} > {pd:e}",
                    cf.delta_star
                );
            }
            prev_delta = Some(cf.delta_star);

            let (x_star, profit) = golden_section_search_2leg(
                U256::from(1u64),
                x1,
                &hop_a,
                &hop_b,
            ...
            // Sign agreement: kernel profitability ⇔ threshold γ1γ2·m/1000 > 1.
            assert_eq!(
                profit > 0,
                cf.numerator > 0.0,
                "m={m}: kernel sign ({profit}) must match closed-form threshold ({})",
                cf.numerator
            );
            if profit > 0 {
                let x_star_f = u256_to_f64_lossy(x_star);
                if let Some(px) = prev_x_star {
                    // Adjacent Δ* rungs differ by ≥3.5× — far beyond golden
                    // resolution (≈5.5e-4 relative), so strict ordering is safe.
                    assert!(
                        x_star_f > px * 1.05,
                        "kernel size must track Δ* upward (m={m}): {x_star_f:e} vs {px:e}"
                    );
                }
                prev_x_star = Some(x_star_f);
                assert!(
                    profit as f64 <= cf.net_star * (1.0 + 1e-9),
                    "m={m}: envelope violated vs closed form"
                );
                assert!(
                    profit as f64 >= cf.net_star * 0.99,
                    "m={m}: kernel harvests <99% of Net*"
                );
            }
        }
    }

    /// BR-06 gap #3 (fee structure — CORRECTED symmetry): fees enter the
    /// numerator only via √(γ1·γ2) (product, not split) and gate existence
    /// through the threshold γ1γ2·(y1·x2)/(x1·y2) > 1; but Δ* itself is NOT
    /// exchange-invariant (γ1 sits alone in the denominator) — explicit
    /// counterexample below refutes the literal wording of
    /// BR-05+06-VERIFY §2.2 check 3.
    #[test]
    fn br06_fee_enters_via_geometric_mean_threshold_not_split() {
        let (hop_a, hop_b) = br06_hop_pair(1050);
        let (x1, y1) = hop_a[0];
        let (y2, x2) = hop_b[0];
        let f = |fa: u32, fb: u32| {
            br06_closed_form_2leg(
                u256_to_f64_lossy(x1),
                u256_to_f64_lossy(y1),
                u256_to_f64_lossy(y2),
                u256_to_f64_lossy(x2),
        ...
        // (a) Product-invariance of the numerator: any split of the same
        // γ1γ2 product — including the on-chain 5-bps leg — shares K.
        let g1 = 1.0 - f64::from(BR06_FEE_V2_BPS) / 10_000.0;
        let g2 = 1.0 - f64::from(BR06_FEE_V3_5_BPS) / 10_000.0;
        let b_split_bps = (10_000.0 * (1.0 - g1 * g2)) as u32; // (0, b): same product
        let n_ab = f(BR06_FEE_V2_BPS, BR06_FEE_V3_5_BPS).numerator;
        let n_ba = f(BR06_FEE_V3_5_BPS, BR06_FEE_V2_BPS).numerator;
        let n_0b = f(0, b_split_bps).numerator;
        assert!((n_ab - n_ba).abs() / n_ab.abs() < 1e-12);
        assert!((n_ab - n_0b).abs() / n_ab.abs() < 1e-12);

        // (b) Threshold gates existence, kernel agrees with the sign: a 0.1%
        // dislocation is below the fee wall for (30,30) and (5,30), above it
        // at zero fee — the classic fee-adjusted no-arbitrage band.
        let (ha_t, hb_t) = br06_hop_pair(1001);
        let (x1t, y1t) = ha_t[0];
        let (y2t, x2t) = hb_t[0];
        for (fa, fb) in [(30u32, 30u32), (5, 30), (0, 0)] {
            let cf = br06_closed_form_2leg(
                u256_to_f64_lossy(x1t),
                u256_to_f64_lossy(y1t),
                u256_to_f64_lossy(y2t),
                u256_to_f64_lossy(x2t),
                fa,
                fb,
            );
            let (_, profit) = golden_section_search_2leg(
            ...
            assert_eq!(
                profit > 0,
                cf.numerator > 0.0,
                "fees ({fa},{fb}): kernel sign {profit} must match threshold {}",
                cf.numerator
            );
        }

        // (c) COUNTEREXAMPLE to full Δ* exchange-invariance: balanced math
        // vector (SYNTHETIC — labeled; no on-chain claim), split fees
        // (30,5) vs (5,30): numerators equal, Δ* differs by 1.67e-3 — the
        // γ1 in the denominator is real. Hand-derived: Δ*(30,5)=3.328318085e17,
        // Δ*(5,30)=3.322769039e17.
        let cf_x = br06_closed_form_2leg(1e18, 2e18, 1e18, 2e18, BR06_FEE_V2_BPS, BR06_FEE_V3_5_BPS);
        let cf_y = br06_closed_form_2leg(1e18, 2e18, 1e18, 2e18, BR06_FEE_V3_5_BPS, BR06_FEE_V2_BPS);
        assert!((cf_x.numerator - cf_y.numerator).abs() / cf_x.numerator.abs() < 1e-12);
        let rel = (cf_x.delta_star - cf_y.delta_star).abs() / cf_x.delta_star;
        assert!(
            rel > 1e-4,
            "Δ* is NOT fee-exchange invariant (verify §2.2 check 3 corrected): rel diff {rel:e}"
        );
        assert!((cf_x.delta_star - 3.328_318_085e17).abs() / cf_x.delta_star < 1e-6);

        // (d) Kernel on the on-chain fixture with split fees: both orders
        // profitable (split does not gate existence) and ordered like the
        // closed form (the heavier fee on the FIRST leg sits in the
        // denominator with the smaller γ1 ⇒ larger Δ*).
        let (_, p_ab) = golden_section_search_2leg(
            U256::from(1u64),
            x1,
            &hop_a,
            &hop_b,
        ...
        let (x_ba, p_ba) = golden_section_search_2leg(
            U256::from(1u64),
            x1,
            &hop_a,
            &hop_b,
        ...
        let (x_ab, _) = golden_section_search_2leg(
            U256::from(1u64),
            x1,
            &hop_a,
            &hop_b,
        ...
        assert!(p_ab > 0 && p_ba > 0, "fee split must not gate existence");
        assert!(
            u256_to_f64_lossy(x_ab) >= u256_to_f64_lossy(x_ba),
            "kernel sizes must order like the closed form under fee exchange"
        );
    }

    /// USDC-token_in candidate builder for the decimals-6 end-to-end test
    /// (BR-06): mirrors `make_dex_candidate` with the cycle entering through
    /// USDC — legs[0] USDC→WETH (pool A), legs[1] WETH→USDC (pool B).
    fn make_dex_candidate_usdc_in(pool_a: Address, pool_b: Address) -> StrategyCandidate {
        let id = Uuid::new_v4();
        let pool_a_str = format!("0x{:040x}", pool_a);
        let pool_b_str = format!("0x{:040x}", pool_b);
        // Canonical mainnet addresses (verified on-chain @ block 25,937,800:
        // pool token0()/token1() + token decimals()).
        ...
        let opp = Opportunity {
            ...
            amount_in_wei: "1000000".to_string(), // 1 USDC (6 dec)
            expected_profit_usd: Some(1.0),
            net_expected_profit_usd: None,
            roi_pct: None,
            risk_score: None,
            block_number: Some(BR06_BLOCK),
            rejection_reason: None,
            cartridge_id: None,
            detected_at: Utc::now(),
            trace_id: Uuid::new_v4(),
        ...
        let candidate_inner = OpportunityCandidate {
            route_fingerprint: "test".to_string(),
            pool_addresses: vec![pool_a_str.clone(), pool_b_str.clone()],
            token_addresses: vec![usdc.clone(), weth.clone()],
            dex_adapters: vec!["uniswap-v2".to_string(), "sushi".to_string()],
            amount_in: 1.0,
            expected_amount_out: 1.001,
            gross_profit: 1.0,
        ...
        let route_plan = RoutePlan {
            route_id: Some("test-route-usdc".to_string()),
            strategy_kind: StrategyLabel::DexArbV2V2.as_str().to_string(),
            chain_id: 1,
            legs: vec![
                RouteLeg {
                    dex_id: "uniswap-v2".to_string(),
    ...
    /// BR-06 gap #4 (decimals 6/18 through the REAL optimizer path): the
    /// full `optimize_with_reason` pipeline on a USDC-token_in cycle —
    /// cap_usd→cap_wei through 10^6, string-oriented reserves, per-leg
    /// fees, integer kernel, profit converted back through 10^6 — must
    /// land on the closed-form Δ* and report USD figures consistent with
    /// the integer wei ledger (HOPS-LEDGER-04).
    #[tokio::test]
    async fn br06_usdc6_weth18_end_to_end_sizing_closed_form() {
        let pool_a = addr(0x10);
        let pool_b = addr(0x11);
        let tok_usdc = addr(0xAAAA);
        let tok_weth = addr(0xBBBB);

        let cache = Arc::new(ReservesCache::new());
        // Pool A: real V2 USDC/WETH reserves (block 25,937,800) stored as
        // (r0, r1) like the pool_sync writer: token0=USDC, token1=WETH.
        let r0 = br06_v2_r0_usdc();
        let r1 = br06_v2_r1_weth();
        cache.insert(pool_a, r0, r1).await;
        // Pool B: DECLARED synthetic dislocation — WETH side ×980/1000
        // (USDC side unchanged), the "other venue" of the cycle.
        let r1_b = r1 * U256::from(980u64) / U256::from(1000u64);
        cache.insert(pool_b, r0, r1_b).await;

        // Closed form for USDC→WETH→USDC: x1 = USDC side of A, y1 = WETH
        // side of A, y2 = WETH side of B, x2 = USDC side of B.
        let cf = br06_closed_form_2leg(
            u256_to_f64_lossy(r0),
            u256_to_f64_lossy(r1),
            u256_to_f64_lossy(r1_b),
            u256_to_f64_lossy(r0),
            BR06_FEE_V2_BPS,
            BR06_FEE_V2_BPS,
        );
        // Hand-derived vector (exact Python port, 2026-09-09):
        // Δ* = 3.572794135e10 USDC-wei ($35,727.94), Net* = 2.544575037e8
        // USDC-wei ($254.46). Cap $500K is non-binding (Δ* < 5e11).
        assert!(cf.delta_star > 0.0);
        assert!((cf.delta_star - 3.572_794_135e10).abs() / cf.delta_star < 1e-6);

        let projector = Arc::new(StateProjector::new(cache, None));
        let optimizer = SizeOptimizer::new(projector);
        let candidate = make_dex_candidate_usdc_in(pool_a, pool_b);
        let intent = make_intent(tok_usdc, tok_weth);
        // Kelly permissive (mult 1.0, no per-trade cap, p 0.9): cap ≈ $447K
        // ≫ Δ* ⇒ the kernel answer must survive Step 8 untouched.
        let cfg = make_cfg_kelly(500_000.0, 1.0, 1.0, 1.0, 0.9);

        let outcome = optimizer
            .optimize_with_reason(candidate, &intent, Some(&cfg))
            .await
            .expect("optimize must not error");
        let OptimizeOutcome::Sized(sized) = outcome else {
            panic!("USDC-in cycle must size, got {:?}", outcome.reason_str());
        };

        let amount_f = u256_to_f64_lossy(sized.optimal_amount_in);
        let loc_rel = (amount_f - cf.delta_star).abs() / cf.delta_star;
        assert!(
            loc_rel < 5e-3,
            "end-to-end amount {amount_f:e} must sit on Δ* {:e} (rel {loc_rel:e})",
            cf.delta_star
        );
        // Cap non-binding: the optimum is interior (< cap_wei = 5e11).
        assert!(sized.optimal_amount_in < U256::from(500_000_000_000u64));

        // Decimals-6 integrity: gross_usd × 10^6 == integer net wei of the
        // SAME kernel evaluation at the reported size (orientation check:
        // legs[0] USDC in ⇒ (r0, r1); legs[1] WETH in ⇒ (r1_b, r0)).
        let hop_a = vec![(r0, r1)];
        let hop_b = vec![(r1_b, r0)];
        let profit_int = eval_2leg_profit(
            sized.optimal_amount_in,
            &hop_a,
            &hop_b,
            BR06_FEE_V2_BPS,
        ...
        assert!(profit_int > 0);
        assert!(
            (sized.gross_profit_usd * 1e6 - profit_int as f64).abs() <= 1.0,
            "gross_usd {} must be profit_wei {profit_int} scaled by 10^-6",
            sized.gross_profit_usd
        );
        // Own capital (no base_strategy): net = gross − gas(=$12 cfg) − 0 − 0.
        assert!((sized.estimated_net_profit_usd - (sized.gross_profit_usd - 12.0)).abs() < 1e-6);

        // HOPS-LEDGER-04: per-leg wei chain at the reported size — leg 1's
        // input IS leg 0's output (integer-identical).
        let legs_in = sized.leg_amounts_in.as_ref().expect("ledger present");
        let legs_out = sized.leg_amounts_out.as_ref().expect("ledger present");
        assert_eq!(legs_in[0], sized.optimal_amount_in.to_string());
        let out_a = v2_amount_out(sized.optimal_amount_in, r0, r1, BR06_FEE_V2_BPS);
        assert_eq!(legs_in[1], out_a.to_string());
        assert_eq!(legs_out[0], out_a.to_string());
        let out_b = v2_amount_out(out_a, r1_b, r0, BR06_FEE_V2_BPS);
        assert_eq!(legs_out[1], out_b.to_string());
    }

    /// BR-06 gap #5 (Kelly caps, exact ratio upgrade): when the Kelly
    /// budget binds, the reported size is EXACTLY floor(cap_usd·f_capped /
    /// price · 10^decimals) — not merely "smaller" — in both the
    /// net-surviving and the net-negative arms; and across a parameter
    /// sweep a Sized outcome NEVER exceeds the Kelly cap (cap-violation
    /// property).
    #[test]
    fn br06_kelly_caps_exact_amount_and_never_exceeded() {
        // Case A — kelly_max_per_trade_fraction binds, scaled net dies:
        // kernel 1 WETH, gross $100 / net $90 (cost $10), p=0.8 ⇒ W=10,
        // f_raw=0.78, ×0.5=0.39, min(0.39, 0.001)=0.001 ⇒ cap_usd $3 ⇒
        // floor(3/3000·10^18) = 10^15 wei EXACTLY; new_gross = $0.1 < cost
        // ⇒ net_negative arm (KELLY-CAPS-01: amount rebinds to the cap).
        let cfg_a = make_cfg_kelly(3_000.0, 0.5, 0.001, 1.0, 0.8);
        let sized_a = make_sized(unit(1), 100.0, 90.0);
        match SizeOptimizer::apply_kelly_constraints(
            OptimizeOutcome::Sized(Box::new(sized_a)),
            &cfg_a,
            cfg_a.capital_usd,
            3000.0,
            18,
            ...
            OptimizeOutcome::Sized(s) => {
                assert_eq!(
                    ...
                    "Kelly bind must rebind EXACTLY to floor(cap·f/price·10^d)"
                );
                assert!(s.net_negative, "scaled net ≤ 0 must be flagged net_negative");
                assert!((s.gross_profit_usd - 0.1).abs() < 1e-9);
                assert_eq!(s.estimated_net_profit_usd, 0.0);
                assert!(s.leg_amounts_in.is_none() && s.leg_amounts_out.is_none());
            }
            other => panic!("case A must Size, got {:?}", other.reason_str()),
        }

        // Case B — plain f*·nav cap binds, scaled net survives: f_raw=0.78
        // (mult 1.0) ⇒ cap 0.78 WETH = floor(0.78·10^18) wei; ratio 0.78 ⇒
        // gross 100→78, net 78−10=68 > gas floor.
        let cfg_b = make_cfg_kelly(3_000.0, 1.0, 1.0, 1.0, 0.8);
        let sized_b = make_sized(unit(1), 100.0, 90.0);
        match SizeOptimizer::apply_kelly_constraints(
            OptimizeOutcome::Sized(Box::new(sized_b)),
            &cfg_b,
            cfg_b.capital_usd,
            3000.0,
            18,
            ...
            OptimizeOutcome::Sized(s) => {
                // Recompute the documented formula independently — the
                // contract is bit-exact agreement with it.
                let f_raw: f64 = 0.8 - 0.2 / (100.0 / 10.0);
                let f_capped: f64 = (f_raw * 1.0).clamp(0.0, 1.0).min(1.0);
                let expected =
                    f64_to_u256_clamped(((3_000.0 * f_capped) / 3000.0 * 1e18).floor());
                assert_eq!(s.optimal_amount_in, expected);
                assert!(
                    (u256_to_f64_lossy(s.optimal_amount_in) - 7.8e17).abs() < 1e3,
                    "expected ≈0.78 WETH, got {}",
                    s.optimal_amount_in
                );
                assert!((s.gross_profit_usd - 78.0).abs() < 1e-9);
                assert!((s.estimated_net_profit_usd - 68.0).abs() < 1e-9);
                assert!(!s.net_negative);
                assert!(s.leg_amounts_in.is_none() && s.leg_amounts_out.is_none());
            }
            other => panic!("case B must Size, got {:?}", other.reason_str()),
        }

        // Property sweep — a Sized outcome NEVER exceeds the Kelly cap.
        let kernels = [
            (unit(1), 100.0_f64, 90.0_f64),
            (unit(10), 10_000.0, 9_000.0),
            (unit(1), 50.0, 30.0),
        ];
        for (amount, gross, net) in kernels {
            for mult in [0.05_f64, 0.5, 1.0, 2.0] {
                for max_frac in [0.0005_f64, 0.01, 1.0] {
                    for p in [0.5_f64, 0.8, 0.99] {
                        let cfg = make_cfg_kelly(3_000.0, mult, max_frac, 1.0, p);
                        let cost = (gross - net).max(0.0);
                        let w = if cost > 1e-9 { gross / cost } else { f64::INFINITY };
                        let p_c = p.clamp(0.01, 0.99);
                        let f_raw = if w.is_infinite() { p_c } else { p_c - (1.0 - p_c) / w };
                        let f_capped = (f_raw * mult).clamp(0.0, 1.0).min(max_frac);
                        let cap_wei = f64_to_u256_clamped(
                            ((3_000.0 * f_capped) / 3000.0 * 1e18).floor(),
                        );
                        match SizeOptimizer::apply_kelly_constraints(
                            OptimizeOutcome::Sized(Box::new(make_sized(amount, gross, net))),
                            &cfg,
                            cfg.capital_usd,
                            3000.0,
                            18,
                            ...
                            OptimizeOutcome::Sized(s) => {
                                assert!(
                                    s.optimal_amount_in <= cap_wei + U256::one(),
                                    "Kelly cap violated: {} > {cap_wei} (mult={mult}, max={max_frac}, p={p})",
                                    s.optimal_amount_in
                                );
                            }
                            OptimizeOutcome::Rejected(r) => {
