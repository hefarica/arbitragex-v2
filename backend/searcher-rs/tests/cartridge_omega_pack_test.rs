...
fn num(m: &mut Map, k: &str, v: f64) { m.insert(k.into(), Dynamic::from(v)); }
fn int(m: &mut Map, k: &str, v: i64) { m.insert(k.into(), Dynamic::from(v)); }
fn s(m: &mut Map, k: &str, v: &str) { m.insert(k.into(), Dynamic::from(v.to_string())); }
fn boolean(m: &mut Map, k: &str, v: bool) { m.insert(k.into(), Dynamic::from(v)); }

fn is_opp(r: &Map) -> bool { r.get("is_opportunity").unwrap().as_bool().unwrap() }
fn reason(r: &Map) -> String { r.get("reason").unwrap().clone().into_string().unwrap() }
fn kind(r: &Map) -> String { r.get("strategy_kind").unwrap().clone().into_string().unwrap() }
...
fn omega_metadata_shadow_zero_capital() {
    ...
    let mut scope = Scope::new();
    let meta: Map = e.call_fn::<Dynamic>(&mut scope, &ast, "init_strategy", ())
        .unwrap().cast::<Map>();
    for k in ["name", "version", "author", "description"] {
        assert!(meta.contains_key(k), "metadata missing `{k}`");
    }
    assert_eq!(meta.get("mode").unwrap().clone().into_string().unwrap(), "shadow");
...
fn omega_spatial_cross_dex_profitable() {
    ...
    let r = run("evaluate_opportunity", d);
    assert!(is_opp(&r), "10 USD gross minus ~3 USD costs must clear threshold");
...
fn omega_post_block_backrun_rejects_pending_tx() {
    ...
    let r = run("evaluate_opportunity", d);
    assert!(!is_opp(&r));
    assert_eq!(reason(&r), "pending_tx_not_supported_by_current_cartridge_surface");
}
...
fn omega_oracle_divergence_is_signal_only() {
    ...
    let r = run("evaluate_opportunity", d);
    assert!(!is_opp(&r), "oracle divergence is a signal, never an executable opportunity");
