...
describe("WO-NO-WS-LOGS-01 — structured ws.* lifecycle logs", () => {
  ...
  it("logs ws.connected at info (grep-friendly event name, socket_id present)", async () => {
    ...
    client.on("connect_error", (e: Error) => console.log("CONNECT_ERROR_T1:", e.message));
    client.on("disconnect", (r: string) => console.log("DISC_T1_EARLY:", r));
    await new Promise<void>((resolve, reject) => {
      client.on("connect", () => resolve());
      setTimeout(() => reject(new Error("t1 timeout")), 12000);
    });
