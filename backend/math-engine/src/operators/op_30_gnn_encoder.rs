...
impl GnnEncoderOperator {
    pub fn new() -> Self { Self { hidden_dim: 16, n_layers: 2 } }

    pub fn encode(&self, node_features: &DMatrix<f64>, adjacency: &HashMap<usize, Vec<usize>>) -> DMatrix<f64> {
        ...
        for _layer in 0..self.n_layers {
            ...
            for v in 0..n_nodes {
                ...
                let neighbor_mean = if neighbor_count > 0 { neighbor_sum / neighbor_count as f64 } else { neighbor_sum };
...
impl TopologicalOperator for GnnEncoderOperator {
    fn id(&self) -> u8 { 30 }
    fn name(&self) -> &'static str { "GNN Encoder" }
    fn category(&self) -> &'static str { "ml" }

    fn evaluate(&self, state: &MarketState) -> OperatorOutput {
        let n = state.price_matrix.len();
        if n == 0 {
            return OperatorOutput {
                operator_id: self.id(), operator_name: self.name().to_string(),
                scalar_value: None, vector_result: None, matrix_result: None, metadata: HashMap::new(),
            };
        }
        let m = state.price_matrix[0].len();
        let mut node_data = Vec::with_capacity(n * m);
        for row in &state.price_matrix { for &val in row { node_data.push(val); } }
        let node_features = DMatrix::from_row_slice(n, m, &node_data);

        let mut adjacency = HashMap::new();
        for v in 0..n { adjacency.insert(v, vec![(v + n - 1) % n, (v + 1) % n]); }

        let embeddings = self.encode(&node_features, &adjacency);

        let mut avg_embedding = nalgebra::DVector::zeros(self.hidden_dim);
        for v in 0..n {
            for i in 0..self.hidden_dim { avg_embedding[i] += embeddings[(v, i)]; }
        }
        ...
        OperatorOutput {
            operator_id: self.id(), operator_name: self.name().to_string(),
            scalar_value: Some(embedding_norm), vector_result: Some(avg_embedding.iter().cloned().collect()),
            matrix_result: None, metadata,
        }
    }
}
