...
impl NashOperator {
    ...
    pub fn solve_pure_2x2(row_payoffs: &DMatrix<f64>, col_payoffs: &DMatrix<f64>) -> Option<(usize, usize)> {
        ...
        for r in 0..rows {
            for c in 0..cols {
                ...
                if row_best && col_best { return Some((r, c)); }
            }
        }
        None
    }

    pub fn solve_mixed_2x2(row_payoffs: &DMatrix<f64>, col_payoffs: &DMatrix<f64>) -> Option<(f64, f64)> {
        let a = row_payoffs;
        let b = col_payoffs;
        let denom_col = a[(0, 0)] - a[(0, 1)] - a[(1, 0)] + a[(1, 1)];
        if denom_col.abs() < 1e-12 { return None; }
        let p = (a[(1, 1)] - a[(0, 1)]) / denom_col;
        if !(0.0..=1.0).contains(&p) { return None; }
        let denom_row = b[(0, 0)] - b[(1, 0)] - b[(0, 1)] + b[(1, 1)];
        if denom_row.abs() < 1e-12 { return None; }
...
impl TopologicalOperator for NashOperator {
    fn id(&self) -> u8 { 24 }
    fn name(&self) -> &'static str { "Nash" }
    fn category(&self) -> &'static str { "game_theory" }
    ...
    fn evaluate(&self, _state: &MarketState) -> OperatorOutput {
        ...
        metadata.insert("has_pure".to_string(), if pure.is_some() { 1.0 } else { 0.0 });
        metadata.insert("has_mixed".to_string(), if mixed.is_some() { 1.0 } else { 0.0 });
        ...
        OperatorOutput {
            ...
            scalar_value: mixed.map(|(p, _)| p), vector_result: None,
