...
mod tests {
    ...
    // ── Op-32 canónico: NSGA-II (alta topología) — contrato del registry ─────
    // Integra la integración #558 (HP-03) con el reemplazo #560: un único brazo
    // registrado (Nsga2Operator). Contrato: features nsga2.* → escalar = tamaño
    // del frente de Pareto + computed=1 + matriz presente; sin objetivos →
    // computed=0 y sin escalar (fail-honest, R8).
    #[test]
    fn registry_dispatches_op_32_nsga2() {
        let registry = OperatorRegistry::new();
        let mut features = HashMap::new();
        features.insert("nsga2.count".to_string(), 3.0);
        features.insert("nsga2.population_size".to_string(), 2.0);
        // Tres candidatos: {0,1} no dominados entre sí; {2} dominado por ambos.
        features.insert("nsga2.0.net_profit_usd".to_string(), 10.0);
        features.insert("nsga2.0.risk_cvar_usd".to_string(), 5.0);
        features.insert("nsga2.0.latency_ms".to_string(), 100.0);
        features.insert("nsga2.1.net_profit_usd".to_string(), 12.0);
        features.insert("nsga2.1.risk_cvar_usd".to_string(), 4.0);
        features.insert("nsga2.1.latency_ms".to_string(), 120.0);
        features.insert("nsga2.2.net_profit_usd".to_string(), 9.0);
        features.insert("nsga2.2.risk_cvar_usd".to_string(), 6.0);
        ...
        let honest = registry.dispatch(32, &empty).unwrap();
        assert!(honest.scalar_value.is_none(), "sin objetivos ⇒ sin escalar");
        assert_eq!(honest.metadata.get("computed"), Some(&0.0));
    }
