...
//!   // BR-05 (2026-09-07): reconciled with main's S4-03 label semantics
//!   // (WO-07-DESIGN §3.2, Edición 1).
...
/// Θ clamp keeps logit finite: each clamped logit ∈ [−ln(9999), +ln(9999)] ≈
/// ±9.21, so a log-LR (a DIFFERENCE of two clamped logits) is bounded by
/// 2·ln(9999) ≈ 18.42 in general (±9.21 only when θ₀ = 0.5, the symmetric
/// case). Math guard, not operator config.
/// // BR-05 (2026-09-07): bound corrected per BR-05+06-VERIFY §1.2 (MINOR-1).
...
async fn tick(db: &PgPool, cfg: &Stage2Config) -> anyhow::Result<()> {
    ...
    let (new_labels,): (i64,) = sqlx::query_as(
        ...
        WHERE actual_timestamp IS NOT NULL
          ...
          AND actual_profit_usd IS NOT NULL
          AND calibration_eligible
          -- BR-05 (2026-09-07): count EXACTLY what consolidate() folds (S4-03
          -- gate + a real label), so unvalued/structurally-ineligible rows
          -- never trigger redundant re-consolidations.
          -- (WO-07-DESIGN §3.2 Edición 3 + MINOR-2 de BR-05+06-VERIFY §1.4.)
        "#,
    )
    .bind(watermark)
    .fetch_one(db)
...
async fn consolidate(db: &PgPool, cfg: &Stage2Config) -> anyhow::Result<()> {
    ...
    let rows: Vec<LabeledRow> = sqlx::query_as::<_, LabeledRow>(
        ...
        WHERE ptr.actual_timestamp IS NOT NULL
          ...
          -- BR-05 (2026-09-07): S4-03 no-contamination gate at the consumer —
