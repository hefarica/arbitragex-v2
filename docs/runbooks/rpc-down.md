...
# Runbook â€” RPC down (mempool detection idle)
...
- Slack: *"No opportunities detected in the last 15m â€” Searcher-rs may be
  idle or kill-switched; verify both."*
- `/status` page shows `searcher-rs` as **UP** (the service is alive) but
  `/opportunities` page is empty.
- Grafana "Detection pipeline": `arbx_searcher_pending_total` rate â‰ˆ 0.
...
1. **Which state is searcher-rs in?**
   ...
   ```
   - `scanner.no_rpc` â†’ `RPC_WS_<chain>` env is empty. Missing credential.
   - `scanner.idle` â†’ env set, but the loop is kill-switched (check
     `/killswitch`).
   - `scanner.connect_error` â†’ env set, but the WS handshake is failing.
   - `scanner.subscription_error` â†’ connected, then lost â€” upstream flapping.
...
3. **Can the VPS reach the provider?**
   ...
   ```
   Non-200 or empty â†’ provider or network. Proceed to remediation A.
   200 with `{"result":"0x...."}` â†’ provider is fine; the WS endpoint is the
   issue. Remediation B.

4. **Is the RPC rate-limiting us?**
   Prometheus: `sum(rate(arbx_rpc_rate_limited_total[5m]))` â€” planned for a
   future PR but not yet in shared-rs/metrics.rs. Until that metric lands,
   grep logs for `429` in searcher-rs output.

## Remediation

### A â€” RPC provider outage or credential wrong
...
### B â€” WS endpoint specifically failing

1. Check the provider's changelog for breaking changes to their WS surface.
2. If they deprecated `newPendingTransactions`, this is a bigger fix â€” open
   an incident, keep the platform in the current idle state, don't "hack"
   around it.

### C â€” Vault sealed / agent not rendering
