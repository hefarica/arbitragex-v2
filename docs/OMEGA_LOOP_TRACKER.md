...
| **G-PRICE-1** | stream precios | **OPEN** | — | — | Sin canal WS de precios: dashboard no recibe snapshot+push (estilo exchange); USD prices solo via polling interno 15-60s. Requisito operador 2026-08-19: ms + snapshot/push. Fix = room WS `prices:<chain>` + Redis pub/sub `arbx:prices:updated:<chain>` desde los 3 writers. | 2026-08-19 |
