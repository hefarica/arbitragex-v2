...
| **G-PRICE-1** | stream precios | **CLOSED-VERIFIED** | #418 `9a5d1a31` + hotfix #419 `5ba170d8` | manual rolling (5 servicios, VPS anclado verificado) | **L4 2026-08-19T18:35Z**: REST edge `/api/prices/live` + directo 200 (369 precios); WS snapshot 6ms post-connect (seq 1); **push `prices:update` 19.5s** (seq 2, ≤ tick worker 30s); invariante XLEN 10004 (delta +4 = flujo natural); log `prices:1 <- price_worker update (369 prices)`. Hotfix #419: call a `subscribeToPriceUpdates` faltante (detectado por L4, no por CI). Residuales tracked: G-PRICE-2/3/5. | 2026-08-19 |
| **G-PRICE-2** | re-precio cards | **OPEN** | — | — | Cards/opportunities existentes nunca re-precian (solo evento `new_opportunity` insert). Requiere `opportunity_updated` — follow-up tras G-PRICE-1. | 2026-08-19 |
| **G-PRICE-3** | Chainlink freshness | **OPEN** | — | — | Chainlink Tier-0 se pollea cada 30s en vez de suscribirse a updates del agregador → data on-chain fresca llega tarde por diseño de consumo. | 2026-08-19 |
| **G-PRICE-4** | cadencias prod | **CLOSED-VERIFIED** | — | — | Verificación live 2026-08-19T17:06-17:09Z: price_worker tick exacto cada 30s (5 chainlink + 33 alchemy + 56 misses/94); hash `arbx:token_prices:1` = 373 fields, TTL 53s; XLEN opps:detected=10000 baseline. Sin overrides de env. | 2026-08-19 |
| **G-PRICE-5** | GeckoTerminal 429 flood | **OPEN** | — | — | token-enricher emite WARN 429 cada ~2.1s sin circuit breaker (a diferencia de CoinGecko CG429-01 con backoff 300s). Log-flood + ciclos desperdiciados. Fix espejo del patrón CG429-01. PR separado (1-PR-1-ID). | 2026-08-19 |
| **G-CI-1** | e2e workflow roto repo-wide | **CLOSED-VERIFIED** | #421 `aa2f9293` | auto-deploy Post-E2E (cadena restaurada) | **L4 doble**: (1) job e2e del propio #421 superó "Set up job" y corrió la suite completa (~15min vs 3s del pin roto); (2) tras rerun por flake apt transitorio: **CI 29/29 pass** — primer run 100% verde con e2e en semanas. Pin `0057852b…` (inexistente) → `0400d5f6…` (tag real v4.2.4, verificado vía API). Resto de pins de ambos workflows validados OK. **Residual**: flake apt-mirror en `install-deps` (1 fallo, rerun lo curó; paso ya es resilient por CI-FLAKE-PLAYWRIGHT-APT). | 2026-08-19 |
| **G-ENV-1** | drift .env VPS | **OPEN** | — | — | `/opt/arbitragex-v2/.env` perdió `ENRICHER_CHAINS`, `NEXT_PUBLIC_EDGE_URL`, `NEXT_PUBLIC_WS_URL` (contenedores vivos los tenían horneados) → cualquier `docker compose` nuevo reventaba en interpolación; causa raíz silenciosa del histórico de deploys no-aplicados. Restituidas 2026-08-19 desde env de contenedores vivos (RULE 00; backup `.env.bak.20260819-gprice1`). Pendiente: (a) por qué se perdieron, (b) `compose.prod.yml` usa default anidado `${NEXT_PUBLIC_WS_URL:-${NEXT_PUBLIC_EDGE_URL/http/ws}}` — interpolación inválita en compose, exige la var explícita. | 2026-08-19 |
