...
# Sprints 7 + 8 + Runtime-real â€” Consolidated Design
...
**Stream A â€” Runtime real (unblocks everything else)**
...
- No fabricated data anywhere â€” if RPC is down, service stays healthy but idle (current `scanner::idle_chain_loop` behavior is preserved).
- Paper-mode stays ON. No capital at risk.

**Stream B â€” Sprint 7 (productive edge + operator UX + secrets)**
...
**Stream C â€” Sprint 8 (observability + reliability + docs)**
...
- E2E tests: Playwright against the compose stack hitting edge â†’ frontend â†’ status page; happy-path + kill-switch + RPC-down.
- Backups: nightly `pg_dump` to a local encrypted volume + rclone-to-B2 with 14-day retention; restore rehearsal script.
- Runbooks: `docs/runbooks/{killswitch,rpc-down,relay-degraded,db-restore,rotate-secrets}.md` â€” each with a "if you see alert X, run commands Y, validate Z".
...
## 3. Stream A â€” Runtime real

### 3.1 RPC provider selection
- **Decision:** Alchemy free tier for both WS and HTTP. 300M compute units/mo is enough to observe mempool at mainnet rates. Fallback: Infura or QuickNode. Keys are injected via Vault â†’ `.env` at container boot, never checked in.
...
### 3.3 Acceptance
- `docker compose --profile sim up -d` â†’ `searcher-rs` logs `scanner.subscribed` (not `scanner.no_rpc`, not `scanner.idle`).
...
- Kill-switch still works: `curl -H 'x-arbx-admin-token: $ADMIN' -XPOST http://localhost:8080/admin/killswitch/on` â†’ logs within 5 s show `scanner.paused`.

---

## 4. Stream B â€” Sprint 7
...
Internet
   â”‚
   â”œâ”€â”€ arbx-edge.<your-domain>.workers.dev          â”€â”€â–º  Worker (read-only, cached)
   â”‚
   â””â”€â”€ ops.<your-domain>        â”€â”€â–º  CF Tunnel â”€â”€â–º  nginx on VPS (admin UI, basic auth)
```
...
- Tunnel: new component `infra/cloudflared/` with a compose service that holds the tunnel credential JSON (mounted from Vault-rendered file). Exposes `ops.<domain>` â†’ `nginx:80`. Nginx routes `/api/admin/*` â†’ `api-server:8080` with TLS terminated at CF, basic-auth at nginx.

**New files**
- `infra/cloudflared/config.yml` â€” tunnel â†’ ingress mapping.
- `infra/cloudflared/Dockerfile` â€” pinned `cloudflare/cloudflared:2024.10.0`.
- `configs/nginx/ops.conf` â€” reverse proxy for admin routes.
- `.github/workflows/edge-deploy.yml` â€” deploys Worker on push to `main` when `edge/worker/**` changes.

### 4.2 Frontend â€” 8 operational pages
...
**Integration** â€” a small init container (`infra/vault-agent/`) renders `.env` from templates at compose `up` and the other services `depends_on` it. If Vault is sealed â†’ init container fails â†’ compose stack fails to start (fail-closed).

**Migration path:** `.env` on disk stays as a development convenience only. `configs/secrets.policy.md` gets a "Vault mode" section.

### 4.4 Acceptance
- `curl -I https://arbx-edge.<user>.workers.dev/health` â†’ 200 with `x-arbx-trace-id` header.
- `curl -I https://ops.<domain>/healthz` via tunnel â†’ 200.
...
1. **Platform overview** â€” `arbx_service_up` per service, `arbx_http_requests_total` rate/status breakdown, `arbx_killswitch_enabled`.
2. **Detection pipeline** â€” `arbx_opportunity_total{status}` rate, dedup hit/miss ratio, scanner reconnect count, decode-failed rate.
3. **Execution pipeline** â€” selector score histogram, sim pass/fail rate, bundle submit latency p50/p95/p99, revert rate.
4. **Recon & risk** â€” PnL over window, anomaly event count, circuit-breaker state table, token-safety cache hit rate.

### 5.2 Alertmanager

Replace the placeholder webhook in `monitoring/alertmanager/alertmanager.yml` with:
- `critical` â†’ PagerDuty (integration key from Vault).
- `warning` â†’ Slack `#arbx-alerts` webhook.
- `info` â†’ Slack only, inhibited 22:00â€“08:00 UTC.
...
- `smoke.spec.ts` â€” all 8 frontend pages load and don't show "edge unreachable".
- `killswitch.spec.ts` â€” toggle via UI, verify `/status` updates, revert.
- `rpc-down.spec.ts` â€” unset `RPC_WS_1`, restart searcher-rs, verify `/status` shows the chain as "idle" without lying about it.
- CI job `.github/workflows/e2e.yml` brings up compose, waits for health, runs Playwright, tears down.

### 5.4 Backups

- `automation/scripts/backup-pg.sh` â€” `pg_dump` â†’ `/var/backups/arbx/pg-<YYYYMMDD-HHMM>.sql.gz`, encrypted with `age` using an operator public key stored at `configs/backup-recipient.age.pub`.
- Cron on VPS (systemd timer, not docker cron): daily 03:15 UTC.
- `automation/scripts/backup-offsite.sh` â€” rclone to Backblaze B2 bucket `arbx-backups` with 14-day retention (rclone lifecycle rule).
- `automation/scripts/restore-pg.sh` â€” takes a file path, decrypts, psql restores to a staging DB name. **Tested** as part of CI on each PR that touches `automation/scripts/backup-*.sh`.

### 5.5 Runbooks

`docs/runbooks/` â€” new dir. Each runbook follows this template:
```
# <alert name / incident>
## Symptoms
...
| Cloudflare account + API token + domain | Tunnel + Worker custom domain | Stream B Â§4.1 |
| Slack incoming webhook URL | Warning/info alerts | Stream C Â§5.2 partial |
| PagerDuty integration key (Events API v2) | Critical alerts | Stream C Â§5.2 partial |
| Backblaze B2 App Key (or S3-compatible) | Off-site backups | Stream C Â§5.4 partial |
| age public key (operator's pubkey) | Encrypted backup envelope | Stream C Â§5.4 |
...
1. **Runtime first.** Stream A lit â€” gives every downstream component real data to work against (dashboards without data are meaningless).
