...
# ArbitrageX v2 â€” Data Requirements Matrix (per module)
...
Columns:
- **Datum** â€” the value the module needs.
- **Type** â€” string / secret / url / address / number / enum / jsonb.
- **Source** â€” allowed source from the doctrine (1â€“8).
- **Sensitivity** â€” public / internal / confidential / secret.
- **Phase** â€” progressive solicitation phase (1â€“5).
- **Validation** â€” how we accept/reject values at intake.
- **Storage** â€” where it rests.
- **Activation condition** â€” what the feature needs to turn on.
- **Block if missing** â€” what the UI/logs say when absent.

Key for Source: `Env`, `Vault`, `DB`, `Form`, `File` (validated config file), `ExtAPI`, `Onboarding`, `UserInput`.

---

## M1 â€” searcher-rs (mempool detection)

| Datum | Type | Source | Sensitivity | Phase | Validation | Storage | Activation | Block if missing |
|-------|------|--------|-------------|-------|------------|---------|-----------|------------------|
| `RPC_WS_<chain_id>` | url | Env â† Vault `secret/arbitragex/<env>/rpc/ws_<chain>` | secret | 2 | `wss://` scheme + successful `eth_chainId` probe matches expected `chain_id` | Vault + env at container boot | chain enabled in `configs/app.toml` AND env present | scanner stays in `idle_chain_loop`, gauge `arbx_service_up{...}=1` but no detection; UI `/status` shows chain as "idle, no RPC" |
| `RPC_HTTP_<chain_id>` | url | Env â† Vault | secret | 2 | `https://` + `eth_blockNumber` probe | Vault | RPC-dependent lookups (tx status, receipts) | non-fatal; recon marks tx status as `unknown` |
| `DATABASE_URL` | url (creds) | Env â† Vault | secret | 1 | connection probe at boot | Vault | persistence on | service runs in "publish-only" mode (stream, no DB); `arbx_db_unavailable=1` |
| `REDIS_URL` | url | Env | internal | 1 | connection + PING | `.env` rendered from Vault template | always | fail-fast at boot (required) |

## M2 â€” sim-ctl (simulation)

| Datum | Type | Source | Phase | Validation | Storage | Activation | Block if missing |
|-------|------|--------|-------|------------|---------|-----------|------------------|
| `ANVIL_FORK_URL` | url | Env â† Vault (may alias `RPC_HTTP_1`) | 3 | `eth_blockNumber` probe | Vault | sim provider = `anvil` | sim-ctl returns `501 {error:"not_implemented", requires:["ANVIL_FORK_URL"]}` |
...
## M3 â€” selector-api (scoring + token safety)
...
| `execution.paper_mode` | bool | File `configs/app.toml` | 4 â†’ 5 flip | schema bool | file | always | **defaults to `true` and stays true through S8**. Flipping to false is an explicit phase-5 operator action with an audit-log entry and a second confirmation. |
...
## M5 â€” recon (reconciliation + learning)

| Datum | Type | Source | Phase | Validation | Storage | Activation | Block if missing |
|-------|------|--------|-------|------------|---------|-----------|------------------|
| `recon.pnl_source_default` | enum | File `configs/app.toml` | 3 | schema | file | always | default `native_only` (no oracles) |
| Chainlink/TWAP oracle addresses | address | DB `price_oracles` editable by admin | 3â€“4 | EVM address + ABI probe | DB | `pnl_source = oracle_*` | recon falls back to `native_only` with warning banner |
| `recon.anomaly_*` thresholds | number | DB `risk_policy` (Phase 0.5) | 4 | bounds check | DB | always | fail-fast on missing row |

## M6 â€” api-server + edge

| Datum | Type | Source | Phase | Validation | Storage | Activation | Block if missing |
|-------|------|--------|-------|------------|---------|-----------|------------------|
| `ARBX_EDGE_TOKEN` | secret | Vault `tokens/edge` | 1 | length â‰¥ 32 bytes base64 | Vault | always | fail-fast boot |
| `ARBX_ADMIN_TOKEN` | secret | Vault `tokens/admin` | 1 | length â‰¥ 32 bytes base64 | Vault | admin endpoints | 401 on admin routes |
| `JWT_SECRET` | secret | Vault `tokens/jwt` | 1 | length â‰¥ 64 bytes | Vault | frontend auth | frontend `/signin` returns `auth_unavailable` |
| `ALLOWED_ORIGINS` | csv | File `configs/app.toml` + env override | 2 | URL list | file | CORS | defaults to empty (no origins) â€” UI shows CORS misconfig notice |
...
## M7 â€” edge Worker + Cloudflare Tunnel
...
## M8 â€” observability (Prometheus, Grafana, Loki, Alertmanager)

| Datum | Type | Source | Phase | Validation | Storage | Activation |
|-------|------|--------|-------|------------|---------|-----------|
| `GRAFANA_ADMIN_PASSWORD` | secret | Vault | 1 | length â‰¥ 16 | Vault | Grafana login | Grafana refuses to start |
| `SLACK_WEBHOOK_URL` | url (secret) | Vault | 2 (optional) â†’ 5 (required) | CF probe (HEAD to webhook) | Vault | severity: `warning`/`info` routes | Alertmanager falls back to webhook-log receiver |
...
## M9 â€” backups & storage

| Datum | Type | Source | Phase | Validation | Storage | Activation |
|-------|------|--------|-------|------------|---------|-----------|
| `AGE_RECIPIENT_PUBKEY` | pubkey string | File `configs/backup-recipient.age.pub` (committed â€” it is a public key) | 5 | age pubkey format | file | encryption | backups remain unencrypted â†’ script refuses to run |
...
| `DATABASE_READONLY_URL` | url (creds) | Env â† Vault | 1 | probe | Vault | read-only dumps | fall back to RW credentials with audit warning |

## M10 â€” chain + router + protocol catalog (canonical)

Lives in code **on purpose** (doctrine Â§"Distinction"):

- Router catalog: `backend/shared-rs/src/chains.rs`.
- Canonical WETH / USDC / â€¦ : currently in scattered literals â€” **to be moved** into `shared-rs/src/tokens.rs` as a catalog in Phase 0.5 remediation.
- Chain RPC method names, ABI fragments â€” not operator-configurable.
...
The operator console (`/onboarding`) â€” to be built in Phase 0.5 â€” walks through phases 1â€“5 in order:

1. **`/onboarding/1-init`** â€” generates tokens, captures Vault unseal keys, sets Grafana admin password.
2. **`/onboarding/2-connect`** â€” captures RPC URLs per chain, optional Slack webhook, test-connects each.
3. **`/onboarding/3-advanced`** â€” sim provider + key, token-safety provider + key, scoring weights sign-off, initial blacklist CSV import.
4. **`/onboarding/4-testing`** â€” relay catalog + per-relay auth, Flashbots signer key (0-balance), execution policy sign-off, CF API token + domain + tunnel.
5. **`/onboarding/5-production`** â€” flips paper-mode off (guarded), PagerDuty key, B2 + age pubkey for backups, incident contacts.
