...
**G2 identificado (segundo generador — el `CREATE INDEX` de AMBOS freezes)**: `database/run_migrations.sh` re-aplica TODAS las migraciones en **cada deploy** (by design, idempotente), y `database/migrations/003_opportunities.sql` crea los índices de `opportunities` con `CREATE INDEX IF NOT EXISTS` **no-concurrent** — que encola ShareLock aunque el índice ya exista. Un deploy que coincide con la ventana del purge reproduce el freeze (04:32:31Z hoy; ayer igual). Fix: (a) los 3 índices de 003 pasan a `CREATE INDEX CONCURRENTLY IF NOT EXISTS` (ShareUpdateExclusiveLock — NO conflictúa con el RowExclusive del hot path); (b) el runner inyecta `lock_timeout=10s` vía `PGOPTIONS` a todo psql → cualquier migración que no obtenga su lock en 10s **aborta el deploy honestamente** (fail-fast, idempotente, reintenta) en vez de encolarse tras la tabla. Sin `statement_timeout` deliberadamente (los builds de índices largos deben terminar).
