...
testMaybe("kill-switch arms and disarms, /status reflects within seconds", async ({ page }) => {
  ...
  // .first() — both the alert-title ("ARMED — executions blocked") and the
  // outcome description ("kill-switch set to ARMED at …") match /armed/i.
  ...
  // .first() — similarly both alert-title and outcome description contain "disabled".
